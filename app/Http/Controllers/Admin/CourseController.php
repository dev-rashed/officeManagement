...
class CourseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $courses = $query->skip($start)->take($length > 0 ? $length : 100)->get();
            ...
            $data = $courses->map(function ($course) {
                return [
                    ...
                    'fee' => $course->fee ?? '-',
                    'status' => $course->status === 'published' ? '<span class="dt-badge is-on">Published</span>' : '<span class="dt-badge is-off">Draft</span>',
                    'sort_order' => $course->sort_order,
                    'actions' => '<div class="dt-actions">
                        <a href="' . route('admin.courses.edit', $course->id) . '" class="dt-btn is-edit" title="Edit" aria-label="Edit"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path d="M2.695 14.763l-1.262 3.155a.5.5 0 00.65.649l3.155-1.262a4 4 0 001.343-.885L17.5 5.5a2.121 2.121 0 00-3-3L3.58 13.42a4 4 0 00-.885 1.343z"/></svg></a>
                        <form action="' . route('admin.courses.destroy', $course->id) . '" method="POST" class="dt-del" onsubmit="return confirm(\'Are you sure you want to delete this course?\');">
