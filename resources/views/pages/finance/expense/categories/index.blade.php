<x-layouts::app :title="__('Expense Categories')">
    <div class="space-y-5">

        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-lg font-semibold tracking-tight text-zinc-900 dark:text-white">{{ __('Expense Categories') }}</h1>
                <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">{{ __('Manage reusable expense categories and their current status.') }}</p>
            </div>
            <button
                type="button"
                id="btn-add-category"
        ...
        <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <div class="overflow-x-auto">
                <table
                    id="categories-table"
                    data-ajax-url="{{ route('expense.categories.data') }}"
    ...
    <style>
        ...
        .category-action-btn.is-muted:hover {
            filter: none;
        }
    ...
    <div id="category-modal" class="fixed inset-0 z-[110] hidden items-center justify-center p-4" aria-hidden="true">
        ...
        <div class="relative w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-zinc-800 dark:bg-zinc-900">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <h2 id="modal-title" class="text-sm font-semibold text-slate-900 dark:text-white">{{ __('Add Expense Category') }}</h2>
            ...
            <div class="mt-5 space-y-4">
                <div>
                    ...
                    <input
                        ...
                        placeholder="{{ __('e.g. Office Rent') }}"
                    >
                    <p id="cat-name-error" class="mt-1.5 hidden text-xs text-rose-400"></p>
                </div>

                <div>
                    <label for="cat-code" class="text-xs font-medium text-slate-700 dark:text-zinc-300">{{ __('Reporting Code') }} <span class="text-slate-400">({{ __('optional') }})</span></label>
                    <input
                        id="cat-code"
                        type="text"
                        class="mt-1.5 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 dark:border-zinc-700 dark:bg-zinc-950 dark:text-white dark:focus:ring-sky-500/30"
                        placeholder="{{ __('e.g. OFF-RENT') }}"
                    >
                    <p id="cat-code-error" class="mt-1.5 hidden text-xs text-rose-400"></p>
                </div>

                <div>
                    <label for="cat-description" class="text-xs font-medium text-slate-700 dark:text-zinc-300">{{ __('Description') }} <span class="text-slate-400">({{ __('optional') }})</span></label>
                    <textarea
                        id="cat-description"
                        rows="2"
                        class="mt-1.5 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 dark:border-zinc-700 dark:bg-zinc-950 dark:text-white dark:focus:ring-sky-500/30"
                        placeholder="{{ __('What belongs in this category') }}"
                    ></textarea>
                </div>
    ...
    <script>
        ...
        const STORE_URL = @js(route('expense.categories.store'));
        ...
        const catCodeErr = document.getElementById('cat-code-error');
        ...
        const clearErrors = () => {
            ...
            catCodeErr.textContent = '';
            catCodeErr.classList.add('hidden');
            catCode.classList.remove('border-rose-500');
        };
        ...
        const showCodeError = (msg) => {
            catCodeErr.textContent = msg;
            catCodeErr.classList.remove('hidden');
            catCode.classList.add('border-rose-500');
        };
        ...
        const setMode = (mode, data = {}) => {
            ...
            if (mode === 'create') {
                ...
                modalDesc.textContent = 'Update the category details or change its status.';
                submitBtn.textContent = 'Update Category';
                catName.value = data.name;
                catCode.value = data.code ?? '';
        ...
        submitBtn.addEventListener('click', async () => {
            ...
            const url = isEdit ? `/expense/categories/${editingId}` : STORE_URL;
            ...
            try {
                ...
                if (res.status === 422) {
                    ...
                    if (errors.code) showCodeError(errors.code[0]);
                    if (!errors.name && !errors.code && json.message) {
