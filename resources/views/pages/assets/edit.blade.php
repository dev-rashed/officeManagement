...
<div class="mx-auto max-w-5xl">
    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-lg font-semibold tracking-tight text-zinc-900 dark:text-white">Edit Asset</h1>
            <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">Update asset assignment, purchase, and lifecycle information.</p>
        </div>
    </div>

    <div class="admin-form-card">
        <form action="{{ route('assets.update', $asset->id) }}" method="POST" enctype="multipart/form-data" class="admin-form">
            @csrf
            @method('PUT')
            ...
            <div class="admin-form-grid">
                ...
                <div class="admin-field">
                    ...
                    <input type="text" id="code_tag_number" name="code_tag_number" value="{{ old('code_tag_number', $asset->code_tag_number) }}" required>
                    @error('code_tag_number') <p class="admin-error">{{ $message }}</p> @enderror
                </div>

                <div class="admin-field">
                    <label for="category">Category</label>
                    <input type="text" id="category" name="category" value="{{ old('category', $asset->category) }}" required>
                ...
                <div class="admin-field">
                    ...
                    <input type="date" id="purchase_date" name="purchase_date" value="{{ old('purchase_date', optional($asset->purchase_date)->format('Y-m-d')) }}">
                    @error('purchase_date') <p class="admin-error">{{ $message }}</p> @enderror
                </div>

                <div class="admin-field">
                    <label for="purchase_cost">Purchase Cost</label>
                    <input type="number" id="purchase_cost" name="purchase_cost" step="0.01" min="0" value="{{ old('purchase_cost', $asset->purchase_cost) }}">
                    @error('purchase_cost') <p class="admin-error">{{ $message }}</p> @enderror
                </div>

                <div class="admin-field">
                    <label for="current_value">Current Value</label>
                    <input type="number" id="current_value" name="current_value" step="0.01" min="0" value="{{ old('current_value', $asset->current_value) }}">
                    @error('current_value') <p class="admin-error">{{ $message }}</p> @enderror
                </div>

                <div class="admin-field">
                    <label for="vendor_supplier">Vendor/Supplier</label>
                    <input type="text" id="vendor_supplier" name="vendor_supplier" value="{{ old('vendor_supplier', $asset->vendor_supplier) }}">
                ...
                <div class="admin-field">
                    ...
                    <select id="assigned_user_id" name="assigned_user_id">
                        ...
                        @foreach($users as $id => $name)
                            <option value="{{ $id }}" @selected((string) old('assigned_user_id', $asset->assigned_user_id) === (string) $id)>{{ $name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="admin-field">
                    ...
                    @if($asset->attachment_path)
                        <a href="{{ Storage::disk('public')->url($asset->attachment_path) }}" target="_blank" class="admin-help-link">View current attachment</a>
                    @endif
            ...
            <div class="admin-form-actions">
                ...
                <button type="submit" class="admin-btn admin-btn-primary">Update Asset</button>
            </div>
        </form>
    </div>
</div>
